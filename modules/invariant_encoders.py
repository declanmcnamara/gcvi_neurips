...
class SetTransformer(nn.Module):
    ...
    '''
    def __init__(self, n_out, hidden_dim, num_heads, parameterization):
        super(SetTransformer, self).__init__()
        self.enc = nn.Sequential(
            SAB(dim_in=1, dim_out=hidden_dim, num_heads=num_heads),
    ...
    def forward(self, x_in):
        #x = x.transpose(0,1)
        latent = self.enc(x_in)
        out = self.dec(latent)

        if self.parameterization == 'mean':
            means = out[...,0]
            log_sds = out[...,1].clamp(min=-10., max=0.)
            to_return = torch.cat([means.T, torch.ones_like(log_sds.exp()).T], -1)
            return to_return
            ...
            means = out[...,0]
            log_sds = out[...,1].clamp(min=-10., max=0.)
            to_return = torch.cat([means.T, log_sds.exp().T], -1)
            return to_return
